...
*/
    ...
    if( isset($_POST['coderlol_update_note_btn']) )
    {
        ...
        $row = $wpdb->get_row("SELECT id, title, content, color, fontSize, category, rotation, grad, gradColor, gradPosition FROM ".$table_notes." WHERE id=".$search_data, ARRAY_A);
    ...
    elseif ( isset($_POST['coderlol_preview_note_btn']) )
    {
        ...
        $row = $wpdb->get_row("SELECT date, updateDate FROM ".$table_notes." WHERE id=".$current_note_id, ARRAY_A);
...
function codrlol_ui_subpage_2()
{
    ...
    $result = $wpdb->get_results("SELECT id, title, date FROM ".$table_notes." ORDER BY id DESC", ARRAY_A);
